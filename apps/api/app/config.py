...
class Settings(BaseSettings):
    ...
    jwt_secret: str = Field(
        default="development-only-secret-change-me-now", min_length=32
    )
    access_token_minutes: int = 30
    refresh_token_days: int = 7
    bootstrap_admin_email: str = "administrator@arenamind.local"
    bootstrap_admin_password: str = Field(
        default="ChangeMe-ArenaMind-2026", min_length=12
    )
    ...
    def validate_provider(self):
        ...
        if self.environment == "production":
            ...
            if self.bootstrap_admin_password == "ChangeMe-ArenaMind-2026":
                raise ValueError(
                    "BOOTSTRAP_ADMIN_PASSWORD must be rotated in production"
                )
        return self
